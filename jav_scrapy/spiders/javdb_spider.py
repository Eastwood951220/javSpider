...
from .utils import (_parse_field_value,
                    _should_skip_item,
                    _calculate_magnet_weight_javdb,
                    _prefilter_magnets_javdb,
                    _parse_size)
...
class JavdbSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        try:
            ...
            if not best_magnet:
                ...
                self.logger.info(f"⚠️ {code} | {title} ｜ {url} 没有可用磁力链接，跳过")
                return
    ...
    def get_best_magnet(self, response, only_chinese: bool = False) -> Tuple[str, float, bool]:
        ...
        # 预过滤
        filtered_magnets = _prefilter_magnets_javdb(magnets, only_chinese)
        ...
        for magnet_elem in filtered_magnets:
            ...
            if not magnet_data:
                continue
            weight = _calculate_magnet_weight_javdb(magnet_data)
    ...
    def _parse_magnet_element(self, magnet_elem) -> Optional[Dict]:
        ...
        try:
            ...
            size = _parse_size(meta_text) or 0  # 默认为0

            # 获取标签
            tags = magnet_elem.css(".magnet-name .tags .tag::text").getall() or []
            has_chinese_sub = any("字幕" in tag for tag in tags) or False
